...
all:
	rm -rf release
	mkdir release
	awk '{ system("cd release && git clone https://gerrit.wikimedia.org/r/p/mediawiki/extensions/"$$1".git && cd "$$1" && git checkout "$$2" && cd ../..") }' < externals
...
dev:
	rm -rf dev 
	mkdir dev
	awk '{ system("cd dev && git clone https://gerrit.wikimedia.org/r/p/mediawiki/extensions/"$$1".git && cd ..") }' < externals
